...
###  $\mathrm{O}$ 符号
 ...
 $\Theta$ 符号同时给了我们一个函数的上下界，如果我们只有一个函数的渐进上界的时候，我们使用 $\mathrm{O}$ 符号。对于一个给定的函数 $g(n)$ , 我们把它记作 $\mathrm{O}(g(n))$ 。

$$
\mathrm{O}(g(n)) = \{f(n):存在常数 c,n_0 使得 0\leq f(n) \leq cg(n), \qquad \forall n \geq n_0\}
$$
...
例如快速排序中的“按大小分类”操作，单次执行的最坏时间复杂度，看似是 $\mathrm{O}(n)$ 的。
但是由于快排的分治过程，先前的“分类”操作每次都减小了数组长度，所以实际的总复杂度 $\mathrm{O}(n \log_2 n)$ ，分摊在每一次“分类”操作上，是 $\mathrm{O}(\log_2 n)$ 。
...
因此，若 $p_i = \mathrm{O}(T(n))$ ，则 $\mathrm{O}(T(n))$ 是均摊复杂度的一个下界。
